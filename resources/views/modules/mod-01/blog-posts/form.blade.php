    ...
    <div class="max-w-3xl">
        ...
        <form method="POST"
            ...
            <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 space-y-6">
                <div x-data="{ titleLen: {{ strlen(old('Title', $post->Title)) }} }">
                    <label class="text-xs font-semibold uppercase tracking-wide text-gray-500">Title</label>
                    <input type="text" name="Title" value="{{ old('Title', $post->Title) }}" required maxlength="50"
                        placeholder="e.g. 5 stretches to ease desk-work back pain"
                        x-on:input="titleLen = $event.target.value.length"
                    ...
                    <p class="text-xs mt-1" :class="titleLen >= 50 ? 'text-red-500' : 'text-gray-400'">
                        <span x-text="titleLen"></span> / 50 characters
                    </p>
                </div>
                ...
                <div>
                    ...
                    <p class="text-xs text-gray-400 mt-0.5 mb-2">
                        Basic HTML is allowed (e.g. &lt;p&gt;, &lt;b&gt;, &lt;a href="..."&gt;). To make a phrase like
                        "Watch our walkthrough" clickable, wrap it in a link tag, e.g.
                        <code>&lt;a href="https://..."&gt;Watch our walkthrough&lt;/a&gt;</code>.
                    </p>
                ...
                <div>
                    <label class="text-xs font-semibold uppercase tracking-wide text-gray-500">Video URL (optional)</label>
                    <p class="text-xs text-gray-400 mt-0.5 mb-2">
                        Paste a YouTube or Vimeo link to embed the video directly in the post. Any other link
                        (e.g. a Facebook video) will show as a "Watch the video" button that opens in a new tab.
                    </p>
                    <input type="url" name="VideoUrl" value="{{ old('VideoUrl', $post->VideoUrl) }}"
                        placeholder="https://www.youtube.com/watch?v=..."
                        class="w-full rounded-xl border-gray-200 bg-gray-50 focus:bg-white focus:border-indigo-400 focus:ring-indigo-400 transition">
                </div>
